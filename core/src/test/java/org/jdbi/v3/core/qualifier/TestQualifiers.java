...
public class TestQualifiers {
    ...
    public void getQualifiers() throws Exception {
        ...
        assertThat(qualifiers.qualifiers(getClass().getDeclaredField("qualifiedField")))
            .containsExactly(foo(1));

        assertThat(qualifiers.qualifiers(getClass().getDeclaredMethod("qualifiedMethod")))
            .containsExactly(foo(2));

        assertThat(qualifiers.qualifiers(getClass().getDeclaredMethod("qualifiedParameter", String.class).getParameters()[0]))
            .containsExactly(foo(3));

        assertThat(qualifiers.qualifiers(QualifiedClass.class))
            .containsExactly(foo(4));

        assertThat(qualifiers.qualifiers(QualifiedClass.class.getDeclaredConstructor(String.class).getParameters()[0]))
            .containsExactly(foo(5));

        assertThat(qualifiers.qualifiers(getClass().getDeclaredField("twoQualifiers")))
