...
import org.jdbi.v3.core.argument.internal.ClassFieldCache;
...
public class ObjectFieldArguments extends ObjectPropertyNamedArgumentFinder {
    ...
    protected Optional<TypedValue> getValue(String name, StatementContext ctx) {
        Field field = ctx.getConfig(ClassFieldCache.class).getCache()
                .computeIfAbsent(beanClass, x -> Stream.of(beanClass.getFields())
        ...
        try {
            QualifiedType<?> type = QualifiedType.of(field.getGenericType())
                                    .withAnnotations(ctx.getConfig(Qualifiers.class).qualifiers(field));
